 ...
 * @return 0: 常駐していない
 */
int keepchk(tsrarea **pptsrarea) {
  struct _mep *pmep;
  int keepflg =
      _keepchk((struct _mep *)((char *)_dos_getpdb() - sizeof(struct _mep)),
               &g_magic - &g_keepst, &pmep);
  *pptsrarea = (tsrarea *)((char *)pmep + sizeof(struct _mep) +
                           sizeof(struct _psp) + g_tsrarea - g_keepst);
  return keepflg;
}
...
/**
 * @brief 常駐解除
 * @param ptsrarea idhcpc ワークへのポインタ
 * @return 0: 成功
 */
int freepr(const tsrarea *ptsrarea) {
  return _dos_mfree(
      (void *)(ptsrarea - (g_tsrarea - g_keepst) - sizeof(struct _psp)));
}
