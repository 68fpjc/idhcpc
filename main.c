...
static char g_keepmes[] =
	"コンフィギュレーションが完了しました.\n";

static char g_removemes[] =
...
static int discover_dhcp_server(unsigned long *, unsigned long *, struct sockaddr_in *);
static int request_to_dhcp_server(tsrarea *, unsigned long *, char *, unsigned long, unsigned long, struct sockaddr_in *);
static int send_and_receive(dhcp_msg *, struct sockaddr_in *, unsigned long, unsigned long, int);
...
int main(int argc, char *argv[])
{
	...
	/* オプション解析 */
	{
		...
		for (i = 1; i < argc; i++) {
			if ((!strcmp(argv[i], "-r")) || (!strcmp(argv[i], "--release"))) {
				rflag = 1;
			} else if ((!strcmp(argv[i], "-l")) || (!strcmp(argv[i], "--print-lease-time"))) {
				lflag = 1;
	...
	/* ここで常駐チェックしておく */
	keepflag = keepchk((struct _mep *)((void *)_dos_getpdb() - sizeof(struct _mep)),
						&g_magic - &g_keepst, &pmep);
	if (keepflag) {
		/* 常駐部に保存してあった情報をグローバルワークへ転送しておく */
		memcpy(
			&g_tsrarea,
			(void *)pmep + sizeof(struct _mep) + sizeof(struct _psp),
			sizeof(g_tsrarea));
	}
...
static int try_to_keep(int keepflag)
{
	...
	while (1) {
		...
		if ((errno = discover_dhcp_server(
						&me,
						&server,
						&inaddr_s)) != NOERROR) break;
		if ((errno = request_to_dhcp_server(
						&g_tsrarea,
						&g_subnetmask,
						g_domainname,
						me,
...
/* ================================================================
	...
	in:		leasetime	（全体）リース期間（秒）
			startat		IPアドレス設定時のマシン起動時間（秒）
...
/* ================================================================
	...
	in:		pinaddr_s	送信用ソケット情報格納域へのポインタ
	out:	エラーコード
...
/* ================================================================
	...
	DHCPDISCOVERを発行してDHCPOFFERを受信する

	in:		pme			要求IPアドレス格納域へのポインタ
			pserver		DHCPサーバIPアドレス格納域へのポインタ
			pinaddr_s	送信用ソケット情報格納域へのポインタ
	out:	エラーコード

   ================================================================ */
static int discover_dhcp_server(unsigned long *pme, unsigned long *pserver, struct sockaddr_in *pinaddr_s)
{
	int			ret;
	dhcp_msg	msg;

	if ((ret = send_and_receive(&msg, pinaddr_s, 0, 0, DHCPDISCOVER)) != NOERROR) {
		return ret;
	}
...
/* ================================================================
	...
	DHCPREQUESTを発行してDHCPACKを受信する

	in:		ptsrarea	コンフィギュレーション情報格納域へのポインタ
			pmask		サブネットマスク格納域へのポインタ
			pdomain		ドメイン名格納域へのポインタ
			me			要求IPアドレス
			server		DHCPサーバIPアドレス
			pinaddr_s	送信用ソケット情報格納域へのポインタ
	out:	エラーコード

   ================================================================ */
static int request_to_dhcp_server(tsrarea *ptsrarea, unsigned long *pmask, char *pdomain, unsigned long me, unsigned long server, struct sockaddr_in *pinaddr_s)
{
	int			ret;
	dhcp_msg	msg;

	if ((ret = send_and_receive(&msg, pinaddr_s, me, server, DHCPREQUEST)) != NOERROR) {
		return ret;
	}
...
/* ================================================================
	...
	in:		prmsg		DHCPメッセージバッファ格納域へのポインタ
			pinaddr_s	送信用ソケット情報格納域へのポインタ
			me			要求IPアドレス
			server		DHCPサーバIPアドレス
			msgtype_s	DHCPメッセージタイプ（DHCPDISCOVER or DHCPREQUEST）
	out:	エラーコード

   ================================================================ */
static int send_and_receive(dhcp_msg *prmsg, struct sockaddr_in *pinaddr_s, unsigned long me, unsigned long server, int msgtype_s)
{
	...
	for (i=0; i< 5; i++) {
		...
		sendto(g_sock_s, (char *)&smsg, sizeof(smsg), 0, (char *)pinaddr_s, sizeof(*pinaddr_s));
/*		while (socklen(g_sock_s, 1)) ;*/	/* 送信完了待ち */
		...
		while (1) {
			...
			recvfrom(g_sock_r, (char *)prmsg, sizeof(*prmsg), 0, (char *)&inaddr_r, &len);
			...
			if (msgtype_s == DHCPDISCOVER) {
				...
				if ((msgtype_r == DHCPACK) || (msgtype_r == DHCPNAK)) break;	/* 受信完了 */
			}
		}
		if (!timeout) {
			if (g_verbose) {
...
/* ================================================================
	...
	in:		ptsrarea	コンフィギュレーション情報格納域へのポインタ
			pmask		サブネットマスク格納域へのポインタ
			pdomain		ドメイン名格納域へのポインタ
			pmsg		DHCPOFFERまたはDHCPACKで受信したDHCPメッセージ
	out:	エラーコード

   ================================================================ */
static int fill_tsrarea(tsrarea *ptsrarea, unsigned long *pmask, char *pdomain, dhcp_msg *pmsg)
{
...
/* ================================================================
	...
	DHCPRELEASEを発行してネットワークインタフェースを初期化する
...
static int release_config(void)
{
	...
	/* ================================
	   ...
	   ================================ */
	dhcp_make_dhcprelease(&msg, g_tsrarea.me, g_tsrarea.server, &g_macaddr, random());
	if (g_verbose) {
		dhcp_print(&msg);
		printf("DHCPサーバポート（67）へ送信中 ...\n");
		fflush(stdout);
	}
	sendto(g_sock_s, (char *)&msg, sizeof(msg), 0, (char *)&inaddr_s, sizeof(inaddr_s));
...
/* ================================================================
	...
	DHCPDISCOVER時のネットワークインタフェース設定

	in:		ifname	インタフェース名（"en0"）
...
/* ================================================================
	...
	DHCPRELEASE発行後のネットワークインタフェースの辻褄合わせ
